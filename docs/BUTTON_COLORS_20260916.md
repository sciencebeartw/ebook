...
原示範資料含分隔線，但舊聯絡簿 `[上傳課本作業理化第九章]URL` 不含分隔線。顯示時只辨識明確的「上傳課本作業」開頭，將範圍呈現為「上傳課本作業｜理化第九章」；已含分隔線不重複，單獨標題不附空分隔線，其他自訂表單標題保持原文。資料及 URL 不改寫。使用舊格式合成資料，在完整學生頁確認文字、原 URL、390/820/1280px 無水平溢位，並通過語法及 display-options smoke。

## 同日假期卷顯示名稱修正

學生端從既有 `displayOptions.links.holidayLabel` 讀取假期卷按鈕名稱；留空仍顯示「假期練習卷」，自訂名稱會同步套用到題目與答案 PDF（例如「中秋複習卷｜範圍」及「中秋複習卷答案｜範圍」）。輸出會跳脫 HTML，並保留原橘紅預設色、完成／開答案／回報分數流程。完整原版手機頁 390px 驗證無水平溢位。
